...
一个由用户主动调用的解释重置键：补充必要上下文，沿用已有术语，再用清晰、简明、
无歧义的中文讲一遍。

适合上一条回复没有讲明白、但你不想永久改变回复风格的时候。
...
对长任务跑一个对抗式控制回路：每到一个阶段边界，由 Design Challenger 和 Drift
Watchdog 检查 ROI、方案膨胀，以及与原始意图的漂移。它保留已经验证的工作，剪掉不再
服务于目标的动作，只在出现新的验收证据时才继续推进。
