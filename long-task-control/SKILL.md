...
1. **Observe.** Read the actual artifacts, checks, remaining work, history or
   ...
   Do not redo accepted work, and do not finalize synthesis or close until every
   required delegated result is terminal and consumed; optional work may be
   stopped when it can no longer change the outcome.
...
3. **Challenge — run an adversarial pass by default.** At each major phase
   boundary challenge the work on two axes; do not skip this just because the
   phase looks done:
   ...
   actionable, boundary-only, overdesign, or false-positive. Never staple
   reviewer outputs together as the decision.
...
5. **Continue, and close on evidence.** Resume work, but stop and replan the
   ...
   Do not demand terminal-only reports while repeatedly prompting for narration;
   either define meaningful checkpoints or wait using direct telemetry. Interim
   passes review the current phase's diff; the terminal pass before closing
   reviews the **entire cumulative diff**, not just the last phase's. A typical
   rhythm: architecture-review + fix, then general-review + fix, once per module,
   then the same pair twice over the full diff at the end — adjust to the work,
   this is illustrative, not a fixed count. Before a turn, continuation, or
