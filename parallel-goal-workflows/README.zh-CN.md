...
越来越多，真正完成了什么反而不清楚。这个 Skill 会把每个显式调用的顶层目标交给一个
Goal Owner，由它选择直接执行或使用聚焦 helper，最后带着证据和风险回到主会话。
...
- 通过一份干净的本地 brief，把每个被委派的顶层目标交给一个 Goal Owner；
- 由 Goal Owner 直接执行，或只在能改善结果时使用聚焦 helper；
